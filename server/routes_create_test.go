...
func TestCreateAndShowRemoteModel(t *testing.T) {
	...
	if resp.ContextLen != 131072 {
		t.Errorf("context len: expected %d, actual %d", 131072, resp.ContextLen)
	}

	if resp.EmbedLen != 2880 {
		t.Errorf("embed len: expected %d, actual %d", 2880, resp.EmbedLen)
	}
